...
(function() {
  ...
  Morris.Grid = (function(_super) {
    ...
    Grid.prototype.drawYAxisLabel = function(xPos, yPos, text, color) {
      ...
      label = this.raphael.text(xPos, yPos, text).attr('font-size', this.options.gridTextSize).attr('font-family', this.options.gridTextFamily).attr('font-weight', this.options.gridTextWeight).attr('fill', color);
      if (this.options.yLabelAlign === 'right') {
    ...
    Grid.prototype.drawXAxisLabel = function(xPos, yPos, text, color) {
      if (typeof color === 'undefined') {
        color = this.options.gridTextColor;
      }
      return this.raphael.text(xPos, yPos, text).attr('font-size', this.options.gridTextSize).attr('font-family', this.options.gridTextFamily).attr('font-weight', this.options.gridTextWeight).attr('fill', color);
    };
  ...
  Morris.Bar = (function(_super) {
    ...
    Bar.prototype.drawSeries = function() {
      var barWidth, bottom, groupWidth, i, idx, label, lastTop, left, leftPadding, numBars, row, sidx, size, spaceLeft, top, ypos, zeroPos, _i, _ref;
      this.seriesBars = [];
          ...
          _results.push((function() {
            ...
            for (sidx = _k = 0, _len1 = _ref2.length; _k < _len1; sidx = ++_k) {
              ...
              if (ypos !== null) {
                ...
                if (!this.options.horizontal) {
                  lastTop += size;
                  this.seriesBars[idx][sidx] = this.drawBar(left, top, barWidth, Math.max(size, 2), this.colorFor(row, sidx, 'bar'), this.options.barOpacity, this.options.barRadius);
                  if (this.options.inBarValue) {
                    label = this.drawXAxisLabel(left + barWidth / 2, top - this.options.inBarValueMinTopMargin, this.labelContentForRow(idx, sidx), this.options.inBarValueTextColor);
                    if (top - this.options.inBarValueMinTopMargin + label.getBBox().height > bottom) {
                      _results1.push(label.attr('y', bottom - label.getBBox().height, 0));
                    } else {
                  ...
                  lastTop -= size;
                  this.seriesBars[idx][sidx] = this.drawBar(top, left, Math.max(size, 2), barWidth, this.colorFor(row, sidx, 'bar'), this.options.barOpacity, this.options.barRadius);
                  if (this.options.inBarValue) {
                    label = this.drawYAxisLabel(top + size + this.options.inBarValueRightMargin, left + barWidth / 2, this.labelContentForRow(idx, sidx), this.options.inBarValueTextColor);
                    if (size + this.options.inBarValueRightMargin < label.getBBox().width) {
                      _results1.push(label.attr('x', top + label.getBBox().width + Math.abs(this.options.inBarValueRightMargin)));
                    } else {
                      _results1.push(void 0);
                    }
                  } else {
    ...
    Bar.prototype.hilight = function(index) {
      ...
      if (this.seriesBars && this.seriesBars[this.prevHilight] && this.prevHilight !== null && this.prevHilight !== index) {
        ...
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          ...
          if (y) {
            y.animate({
              opacity: this.options.barOpacity
      ...
      if (this.seriesBars && this.seriesBars[index] && index !== null && this.prevHilight !== index) {
        ...
        for (i = _j = 0, _len1 = _ref1.length; _j < _len1; i = ++_j) {
          ...
          if (y) {
            y.animate({
              opacity: this.options.barHighlightOpacity
            }, this.options.highlightSpeed);
          }
        }
      }
    ...
    Bar.prototype.labelContentForRow = function(index, sidx) {
      var content, row, y;
      row = this.data[index];
      content = '';
      y = row.y[sidx];
      content += '' + this.options.labels[sidx] + ':' + (this.yLabelFormat(y)) + ' ';
      if (typeof this.options.labelCallback === 'function') {
        return content = this.options.labelCallback(index, this.options, content, sidx);
      }
    };
    ...
    Bar.prototype.drawBar = function(xPos, yPos, width, height, barColor, opacity, radiusArray) {
      ...
      return path.attr('fill', barColor).attr('opacity', opacity).attr('stroke', 'none');
    };
