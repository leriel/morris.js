    ...
    @bars = for row, idx in @data
      ...
      for ypos, sidx in row._y
        ...
        if ypos != null
          ...
          if not @options.horizontal
            lastTop += size
            @seriesBars[idx][sidx] = @drawBar(left, top, barWidth, Math.max(size, 2), @colorFor(row, sidx, 'bar'),
                @options.barOpacity, @options.barRadius)
          else
            lastTop -= size
            @seriesBars[idx][sidx] = @drawBar(top, left, Math.max(size, 2), barWidth, @colorFor(row, sidx, 'bar'),
                @options.barOpacity, @options.barRadius)
  ...
  hilight: (index) ->
    if @seriesBars && @seriesBars[@prevHilight] && @prevHilight != null && @prevHilight != index
      for y,i in @seriesBars[@prevHilight]
        if y
          y.animate({opacity: @options.barOpacity}, @options.highlightSpeed)

    if @seriesBars && @seriesBars[index] && index != null && @prevHilight != index
      for y,i in @seriesBars[index]
        if y
          y.animate({opacity: @options.barHighlightOpacity}, @options.highlightSpeed)
